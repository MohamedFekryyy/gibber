...
module.exports = function( Gibber, element = '#editor', userEditable=true ) {
  ...
  // Enhanced metronome visual feedback
  setTimeout(() => {
    // Add visual pulse effect to metronome on beat
    const metronomeElement = document.getElementById('metronome');
    
    Gibber.subscribe('metronome.tick', (beat) => {
      // Add active class briefly
      metronomeElement.classList.add('active');
      
      // Remove after a short delay (visual pulse effect)
      setTimeout(() => {
  ...
  // Modified __setup to handle the intro initialization
  cm.__setup = function() {
    if (window.location.search !== '') {
      ...
      if (code.indexOf('join') === 0) {
        ...
        // Hide intro overlay if URL has code
        Templates.hide();
      }
      // else share.js will launch spectator view of live performance
    } else {
      // Initialize templates overlay when there's no URL parameters
      // This way user can select a template
      Templates.init(cm);
    }
  }
